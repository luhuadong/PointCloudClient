...
static void fill_packet(Pandar128Packet *packet)
{
    /* Header */
    ...
    for (int i=0; i<PANDAR128_BLOCK_NUM; i++) {
        packet->blocks[i].Azimuth = htole16(10 * 100);  /* 低字节在前 */

        for (int channel=0; channel<PANDAR128_LASER_NUM; channel++) {
            packet->blocks[i].units[channel].Distance = 100;
    ...
    packet->tail.UTCTime0 = 22;
    packet->tail.UTCTime1 = 6;
    packet->tail.UTCTime2 = 10;
    packet->tail.UTCTime3 = 9;
    packet->tail.UTCTime4 = 30;
    packet->tail.UTCTime5 = 00;

    packet->tail.Timestamp = t.tv_usec;
